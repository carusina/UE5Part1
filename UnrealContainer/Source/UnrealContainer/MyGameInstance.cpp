...
void UMyGameInstance::Init()
{
	...
	UE_LOG(LogTemp, Log, TEXT("순번에 따른 학생 Map의 레코드 수 : % d"), StudentsMap.Num());
	...
	UE_LOG(LogTemp, Log, TEXT("이름에 따른 학생 Map의 레코드 수 : % d"), StudentsMapByUniqueName.Num());
	...
	UE_LOG(LogTemp, Log, TEXT("이름에 따른 학생 MultiMap의 레코드 수 : % d"), StudentsMultiMapByName.Num());

	const FString TargetName(TEXT("이혜은"));
	TArray<int32> AllOrders;
	StudentsMultiMapByName.MultiFind(TargetName, AllOrders);
	UE_LOG(LogTemp, Log, TEXT("이름이 %s인 학생 수 : % d"), *TargetName, AllOrders.Num());

	TSet<FStudentData> StudentsSet; // UStruct를 사용할 땐, 구조체 내에 operater==와 GetTypeHash 정의 필요
	for (int32 ix = 1; ix <= StudentNum; ++ix)
	{
